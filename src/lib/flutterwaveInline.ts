...
function loadFlutterwaveScript() {
  if (typeof window === "undefined") return Promise.reject(new Error("Flutterwave can only open in the browser"));
  ...
  scriptPromise = new Promise<void>((resolve, reject) => {
    ...
    script.onerror = () => reject(new Error("Could not load Flutterwave checkout. Please check your connection and try again."));
...
export async function openFlutterwaveInline(init: FlutterwaveInit, onClose?: () => void) {
  ...
  if (typeof FlutterwaveCheckout !== "function") throw new Error("Flutterwave checkout is unavailable");
  FlutterwaveCheckout({
