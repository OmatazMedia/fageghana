...
function Issued() {
  ...
  const filtered = useMemo(() => {
    return items.filter((c) => {
      ...
      if (
        q &&
        !`${c.full_name} ${c.member_id} ${c.verification_code}`
          .toLowerCase()
        ...
        <div className="flex rounded-full bg-muted p-1">
          {(["all", "active", "expired", "revoked"] as const).map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`rounded-full px-3 py-1 text-xs capitalize ${filter === f ? "bg-primary text-primary-foreground" : ""}`}
            >
              {f}
            </button>
          ))}
              ...
              return (
                <tr key={c.id} className="border-t border-border">
                  ...
                  <td className="px-3 py-2">
                    <span
                      className={`rounded-full px-2 py-0.5 text-xs capitalize ${status === "active" ? "bg-emerald-100 text-emerald-700" : status === "expired" ? "bg-amber-100 text-amber-700" : "bg-destructive/10 text-destructive"}`}
                    >
                      {status}
                    </span>
                  </td>
