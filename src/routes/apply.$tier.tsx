...
function ApplyPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const gateways = (g ?? []).map((row: any) => ({
        ...row,
        config: { public_key: row.public_key },
      }));
      setPlan(p);
      setGateways(gateways);
      setFormSchema((form?.schema as any) ?? []);

      if (token) {
        // Returning from payment with claim token — load pending + check submission
        const { data: paRows } = await supabase.rpc("get_pending_application" as any, {
          _token: token,
        });
        const pa = Array.isArray(paRows) ? paRows[0] : paRows;
        if (pa) {
          setPending(pa);
          if (pa.status === "paid" || pa.status === "claimed") {
            setStep("form");
            return;
          }
        }
  ...
  async function submitContact(e: React.FormEvent) {
    ...
    if (!contact.full_name || !contact.email || !contact.phone)
      return toast.error("All starred fields are required");
    setBusy(true);
    try {
  ...
  async function submitForm(answers: Record<string, any>) {
    ...
    if (!user) {
      setBusy(false);
      return toast.error("Please sign in via the link sent to your email to submit the form.");
    }
  ...
  return (
    <SiteLayout>
      <section className="border-b border-border bg-muted/30 py-8">
        <div className="mx-auto max-w-4xl px-4">
          ...
          <p className="mt-1 text-muted-foreground">
            {plan.currency} {Number(plan.amount).toLocaleString()} · {plan.duration_months} months
          </p>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-4xl px-4 space-y-6">
          ...
          {step === "contact" && (
            ...
            >
              ...
              <p className="text-sm text-muted-foreground">
                We'll create your account automatically once your payment is confirmed.
              </p>
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <Input
                  ...
                  onChange={(v) => setContact((c) => ({ ...c, company_name: v }))}
                />
              </div>
              <button
                disabled={busy}
            ...
            <div className="space-y-4">
                ...
                {singleOnlineGateway ? (
                  <>
                    <h2 className="mb-1 text-lg font-bold">
                      Pay {plan.currency} {Number(plan.amount).toLocaleString()}
                    </h2>
                    <p className="mb-4 text-sm text-muted-foreground">
                      {singleOnlineGateway.provider === "paystack"
                        ? "A secure Paystack checkout will open here."
                        : `You'll be redirected to ${singleOnlineGateway.name} to complete payment securely.`}
                    </p>
                    <button
                      disabled={busy}
                    ...
                    >
                      ...
                      {busy
                        ? "Opening checkout…"
                        : `Proceed to payment with ${singleOnlineGateway.name}`}
                    </button>
                    {manualGateways.length > 0 && (
                      <button
                        onClick={() => setStep("manual")}
                        className="ml-3 text-sm text-muted-foreground underline-offset-4 hover:text-primary hover:underline"
                      >
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                      ...
                      {manualGateways.length > 0 && (
                        <button
                          onClick={() => setStep("manual")}
                          className="flex items-start gap-3 rounded-xl border border-border bg-card p-5 text-left hover:border-primary"
                        >
                          <Banknote className="h-5 w-5 text-primary" />
                          <div>
                            <div className="font-semibold">Manual bank deposit</div>
                            <div className="text-xs text-muted-foreground">
                              Pay into a FAGE bank account, then upload proof.
                            </div>
                          </div>
                        </button>
                      )}
              ...
              <div className="rounded-2xl border border-border bg-card p-6">
                <div className="mb-2 flex items-center gap-2">
                  ...
                  <h3 className="text-lg font-bold">Need the printable form?</h3>
                </div>
                <button
                  onClick={downloadPdf}
                  className="inline-flex items-center gap-2 rounded-full border-2 border-primary px-5 py-2 text-sm font-semibold text-primary hover:bg-primary hover:text-primary-foreground"
                >
          ...
          {step === "manual" && (
            <div className="space-y-4 rounded-2xl border border-border bg-card p-6">
              ...
              {gateways
                .filter((g) => g.provider === "manual_bank")
                .map((g: any) => (
                  <dl key={g.id} className="rounded-lg bg-muted/40 p-4 text-sm">
                    <div className="font-semibold">{g.bank_details?.bank ?? g.name}</div>
                    {g.bank_details?.account_name && (
                      <div>Account: {g.bank_details.account_name}</div>
                    )}
                    {g.bank_details?.account_number && (
                      <div className="font-mono">No: {g.bank_details.account_number}</div>
                    )}
                    {g.bank_details?.branch && <div>Branch: {g.bank_details.branch}</div>}
                  </dl>
                ))}
              <p className="text-sm text-muted-foreground">
                After paying, email proof to{" "}
                <a href={`mailto:${plan.bank_deposit_email}`} className="text-primary underline">
                  {plan.bank_deposit_email}
                </a>
                . Admin will verify and your account will be created.
              </p>
            </div>
          )}
          ...
          {step === "form" && formSchema && (
            <div className="rounded-2xl border border-emerald-200 bg-card p-8">
              <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-emerald-100 px-3 py-1 text-xs font-semibold text-emerald-700">
                <ShieldCheck className="h-3 w-3" /> Payment confirmed
              </div>
              <h2 className="mb-6 text-2xl font-bold">Complete your application</h2>
              {formSchema.length === 0 ? (
                <p className="text-muted-foreground">No form configured yet.</p>
              ) : (
      ...
      <PostDownloadModal
        ...
        message={
          plan.post_download_message ??
          "Complete the form, attach your proof of payment, and email everything to membership@fageghana.org."
        }
      />
    </SiteLayout>
  );
