...
import { Plus, Pencil, Trash2, X, Upload, Bold, Italic, Underline as UnderlineIcon, List, ListOrdered, AlignLeft, AlignCenter, AlignRight, Link as LinkIcon, Image as ImageIcon, Quote, Heading1, Heading2, Tag } from "lucide-react";
...
type NewsRow = {
  ...
  cover_image_url: string | null; published: boolean; published_at: string;
};

const empty: Omit<NewsRow, "id"> = {
  title: "", slug: "", excerpt: "", body: "", category: "",
  author: "FAGE Admin", cover_image_url: "", published: true,
...
function CategoryManager({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    void supabase.from("news").select("category").eq("published", true)
      .then(({ data }) => {
  ...
  async function remove(cat: string) {
    if (!confirm(`Remove category "${cat}"? Articles using it will keep the value but it won't appear in the dropdown.`)) return;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur p-4">
      <div className="w-full max-w-md rounded-2xl bg-card p-6 shadow-xl">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-bold flex items-center gap-2"><Tag className="h-5 w-5 text-primary" /> Manage Categories</h2>
        ...
        <div className="flex gap-2 mb-4">
          ...
          <button onClick={add} className="rounded-xl bg-primary px-4 py-2 text-sm font-semibold text-white cursor-pointer">Add</button>
        </div>
        <ul className="space-y-2 max-h-64 overflow-y-auto">
          {cats.length === 0 && <p className="text-sm text-muted-foreground">No categories yet. Add one above.</p>}
          {cats.map(c => (
            <li key={c} className="flex items-center justify-between rounded-lg border border-border px-3 py-2 text-sm">
              <span className="font-medium">{c}</span>
              <button onClick={() => remove(c)} className="text-destructive hover:bg-destructive/10 rounded p-1 cursor-pointer"><Trash2 className="h-3.5 w-3.5" /></button>
            </li>
          ))}
        </ul>
      </div>
  ...
  return (
    ...
    >
      {loading ? <p className="text-muted-foreground">Loading…</p>
        : rows.length === 0 ? <p className="text-muted-foreground">No articles yet.</p>
        : (
                ...
                {rows.map(r => (
                  <tr key={r.id} className="border-t border-border hover:bg-muted/20">
                    ...
                    <td className="px-4 py-3 text-right">
                      <button onClick={() => setEditing(r)} className="mr-2 rounded p-1.5 hover:bg-accent cursor-pointer"><Pencil className="h-4 w-4" /></button>
                      <button onClick={() => remove(r.id)} className="rounded p-1.5 text-destructive hover:bg-destructive/10 cursor-pointer"><Trash2 className="h-4 w-4" /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
...
function NewsEditor({ initial, isNew, onClose, onSaved }: {
  initial: NewsRow; isNew: boolean; onClose: () => void; onSaved: () => void;
}) {
  ...
  useEffect(() => {
    void supabase.from("news").select("category")
      .then(({ data }) => {
        const unique = Array.from(new Set((data ?? []).map((r: any) => r.category).filter(Boolean))) as string[];
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-auto bg-background/80 p-4 backdrop-blur">
      <div className="my-8 w-full max-w-4xl rounded-2xl bg-card p-6 shadow-xl">
        <div className="mb-6 flex items-center justify-between">
          ...
          <button onClick={onClose} className="rounded p-1 hover:bg-accent cursor-pointer"><X className="h-5 w-5" /></button>
        </div>

        <form onSubmit={save} className="space-y-5">
          <FormField label="Title">
            <input required className={inputCls} value={form.title}
              onChange={e => { update("title", e.target.value); if (isNew && !form.slug) update("slug", slugify(e.target.value)); }} />
          </FormField>
          ...
          <FormField label="Cover image">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <input className={`${inputCls} flex-1`} placeholder="…or paste URL" value={form.cover_image_url ?? ""} onChange={e => update("cover_image_url", e.target.value)} />
            </div>
          </FormField>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <FormField label="Publish date">
              <input type="datetime-local" className={inputCls} value={form.published_at.slice(0, 16)}
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button type="button" onClick={onClose} className="rounded-full px-4 py-2 text-sm hover:bg-accent cursor-pointer">Cancel</button>
            <button type="submit" disabled={busy} className="rounded-full bg-primary px-5 py-2 text-sm font-semibold text-white disabled:opacity-60 cursor-pointer">
