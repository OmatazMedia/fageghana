...
function VerifyPage() {
  ...
  const visible = (mergeLayout(template?.field_positions).verification_display ?? ["name","member_id","tier","issued","expires"]) as FieldKey[];
      ...
      <div className="w-full max-w-lg rounded-2xl bg-card p-8 text-center shadow-lg">
        {!cert ? (
          <>
            ...
            <p className="mt-2 text-sm text-muted-foreground">The code <code className="font-mono">{code}</code> does not match any issued certificate.</p>
          </>
        ) : valid ? (
          <>
            ...
            <p className="mt-1 inline-flex items-center gap-1 text-xs text-muted-foreground"><ShieldCheck className="h-3 w-3" /> Verified by FAGE Ghana</p>
            <div className="mt-6 space-y-2 text-left text-sm">
              {visible.map(k => <Row key={k} label={FIELD_LABELS[k] ?? k} value={valueFor(k)} />)}
            </div>
          </>
        ) : (
          <>
