...
function GatewaysPage() {
  ...
  return (
    <AdminShell
      ...
      description="Configure Paystack, Hubtel, Flutterwave, or manual bank deposit options."
      action={
        <button
          onClick={() => setEditing({})}
            ...
            <div>
              <div className="font-semibold">
                ...
                <span className="ml-2 text-xs text-muted-foreground capitalize">
                  {g.provider.replace("_", " ")}
                </span>
              </div>
              <div className="text-xs text-muted-foreground">Order {g.display_order}</div>
              {g.provider === "paystack" && (
                <div className="mt-1 text-xs text-muted-foreground">
                  Webhook: /api/public/paystack-webhook · Callback: /payment/callback
                </div>
              )}
              {g.provider === "flutterwave" && (
                <div className="mt-1 text-xs text-muted-foreground">
                  Webhook: /api/public/flutterwave-webhook · Callback: /payment/callback
                </div>
              )}
            </div>
            <div className="flex items-center gap-2">
              {(g.provider === "paystack" || g.provider === "flutterwave") && (
                <button
                  ...
                  onClick={() => runGatewayTest(g)}
                  className="rounded-full border border-primary px-3 py-1 text-xs font-semibold text-primary disabled:opacity-60"
                >
              ...
              <button
                onClick={() => toggle(g)}
                className={`rounded-full px-3 py-1 text-xs ${g.enabled ? "bg-emerald-100 text-emerald-700" : "bg-muted"}`}
              >
                {g.enabled ? "Enabled" : "Disabled"}
              </button>
              <button onClick={() => setEditing(g)} className="text-sm text-primary">
                Edit
              </button>
              <button onClick={() => remove(g.id)} className="text-sm text-destructive">
                Delete
              </button>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <form onSubmit={save} className="w-full max-w-lg space-y-3 rounded-2xl bg-card p-6">
            ...
            <FormField label="Display name">
              <input name="name" required className={inputCls} defaultValue={editing.name} />
            </FormField>
            <FormField label="Provider">
              <select
                name="provider"
                className={inputCls}
                defaultValue={editing.provider ?? "paystack"}
              >
            ...
            <details className="rounded-lg bg-muted/30 p-3" open>
              ...
              <FormField label="Public key">
                <input
                  name="public_key"
                  className={inputCls}
                  defaultValue={editing.config?.public_key ?? ""}
                />
              </FormField>
              <FormField label="Secret key">
                <input
                  ...
                  type="password"
                  className={inputCls}
                  defaultValue={editing.config?.secret_key ?? ""}
                />
              </FormField>
              <FormField label="Webhook secret (Flutterwave verif-hash, optional for Paystack)">
                <input
                  name="webhook_secret"
                  type="password"
                  className={inputCls}
                  defaultValue={editing.config?.webhook_secret ?? ""}
                />
              </FormField>
            </details>
            <details className="rounded-lg bg-muted/30 p-3">
              ...
              <FormField label="Bank">
                <input
                  name="bank"
                  className={inputCls}
                  defaultValue={editing.bank_details?.bank ?? ""}
                />
              </FormField>
              <FormField label="Account name">
                <input
                  name="account_name"
                  className={inputCls}
                  defaultValue={editing.bank_details?.account_name ?? ""}
                />
              </FormField>
              <FormField label="Account number">
                <input
                  name="account_number"
                  className={inputCls}
                  defaultValue={editing.bank_details?.account_number ?? ""}
                />
              </FormField>
              <FormField label="Branch">
                <input
                  name="branch"
                  className={inputCls}
                  defaultValue={editing.bank_details?.branch ?? ""}
                />
              </FormField>
            </details>
