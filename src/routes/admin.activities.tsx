...
function ActivitiesAdmin() {
  ...
  async function load() {
    setLoading(true);
    const { data, error } = await supabase.from("activities").select("*").order("event_date", { ascending: false, nullsFirst: false });
  ...
  return (
    <AdminShell
      ...
      action={<button onClick={() => setCreating(true)} className="flex items-center gap-2 rounded-full bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground"><Plus className="h-4 w-4" /> New activity</button>}
    >
      {loading ? (
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} className="border-t border-border">
                  ...
                  <td className="px-4 py-3 text-muted-foreground">{r.event_date ? new Date(r.event_date).toLocaleDateString() : "—"}</td>
...
function ActivityEditor({ initial, isNew, onClose, onSaved }: { initial: ActivityRow; isNew: boolean; onClose: () => void; onSaved: () => void }) {
      ...
      <div className="my-8 w-full max-w-2xl rounded-2xl bg-card p-6 shadow-xl">
        ...
        <form onSubmit={save} className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <FormField label="Event date"><input type="datetime-local" className={inputCls} value={form.event_date ? form.event_date.slice(0, 16) : ""} onChange={(e) => update("event_date", e.target.value ? new Date(e.target.value).toISOString() : null)} /></FormField>
          </div>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <FormField label="Location"><input className={inputCls} value={form.location ?? ""} onChange={(e) => update("location", e.target.value)} /></FormField>
            <FormField label="Spots remaining"><input type="number" className={inputCls} value={form.spots_remaining ?? ""} onChange={(e) => update("spots_remaining", e.target.value ? parseInt(e.target.value) : null)} /></FormField>
          </div>
          <FormField label="Description"><textarea rows={5} className={inputCls} value={form.description} onChange={(e) => update("description", e.target.value)} /></FormField>
          <FormField label="Image">
            <div className="flex items-center gap-3">
              {form.image_url && <img src={form.image_url} alt="" className="h-16 w-24 rounded object-cover" />}
              ...
              <input className={inputCls} placeholder="…or paste URL" value={form.image_url ?? ""} onChange={(e) => update("image_url", e.target.value)} />
            </div>
          </FormField>
