...
const TIER_META: Record<string, { name: string; eyebrow: string; description: string; benefits: string[]; recommended?: boolean }> = {
  ...
  standard: {
    ...
    eyebrow: "GROWING",
    description: "For established exporters who need deeper market insights, advisory access, and stronger advocacy support.",
    benefits: [
            ...
            <div className={`grid grid-cols-1 gap-8 ${plans.length >= 3 ? "md:grid-cols-3" : "md:grid-cols-2"}`}>
              {plans.map((plan) => {
                const meta = TIER_META[plan.tier] ?? {
                  name: `${plan.tier.charAt(0).toUpperCase()}${plan.tier.slice(1)} Membership`,
                ...
                return (
                  <div key={plan.id} className={`relative rounded-3xl p-8 transition ${isRecommended ? "border-2 border-primary bg-card shadow-lg" : "border border-border bg-card shadow-sm"}`}>
                    {isRecommended && (
                    ...
                    <Link to="/apply/$tier" params={{ tier: plan.tier }} className="mt-7 block w-full rounded-full bg-primary py-3 text-center text-sm font-semibold text-primary-foreground transition hover:scale-[1.01]">
                      Apply now
                    </Link>
                    <button onClick={() => handleDownload(plan)} className="mt-2 inline-flex w-full items-center justify-center gap-2 rounded-full border-2 border-primary py-2.5 text-sm font-semibold text-primary hover:bg-primary hover:text-primary-foreground transition">
