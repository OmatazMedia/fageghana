  ...
  head: () => ({
    meta: [
      { title: "Membership — FAGE Ghana" },
      {
        name: "description",
        content:
          "Join Ghana's premier network of exporters. Apply for FAGE Associate, Standard, or Corporate membership.",
      },
...
const TIER_META: Record<
  string,
  { name: string; eyebrow: string; description: string; benefits: string[]; recommended?: boolean }
> = {
  associate: {
    name: "Associate Membership",
    eyebrow: "STARTER",
    description:
      "Ideal for small and medium-sized enterprises looking to break into international markets.",
    benefits: [
  ...
  standard: {
    ...
    eyebrow: "GROWING",
    description: "For established exporters who need deeper market insights and advisory access.",
    benefits: [
      "All Associate benefits included",
      "Sector-specific market reports",
      "Regular advisory clinics",
  ...
  corporate: {
    ...
    description:
      "Designed for established export companies seeking premium benefits and strategic partnerships.",
    benefits: [
...
const benefits = [
  {
    ...
    text: "Connect with verified international buyers and trade partners worldwide.",
  },
  {
    icon: Network,
    title: "Powerful Network",
    text: "Be part of 2,800+ exporters and a federation with deep government and trade ties.",
  },
  {
    icon: TrendingUp,
    title: "Business Growth",
    text: "Access training, financing facilitation, and market intelligence to scale faster.",
  },
  {
    icon: Award,
    title: "Trusted Recognition",
    text: "Members benefit from FAGE's reputation as Ghana's premier export federation.",
  },
];
  ...
  return (
    <SiteLayout>
      ...
      <section className="py-20 scroll-mt-32">
        <div className="mx-auto max-w-7xl px-4">
          <div className="mb-12 text-center">
            ...
            <Reveal variant="up" delay={2}>
              <p className="mx-auto max-w-2xl text-muted-foreground">
                We offer tailored membership categories to support businesses at every stage of
                their export growth.
              </p>
            </Reveal>
          </div>
          {plans.length === 0 ? (
            ...
            <div
              className={`grid grid-cols-1 gap-8 ${plans.length >= 3 ? "md:grid-cols-3" : "md:grid-cols-2"}`}
            >
              {plans.map((plan, i) => {
                const meta = TIER_META[plan.tier] ?? {
                  name: `${plan.tier} Membership`,
                  eyebrow: plan.tier.toUpperCase(),
                ...
                return (
                  <Reveal key={plan.id} variant="up" delay={(i + 1) as 1 | 2 | 3}>
                    <div
                      className={`relative rounded-3xl p-8 h-full flex flex-col transition ${isRec ? "border-2 border-primary bg-card shadow-lg" : "border border-border bg-card shadow-sm"}`}
                    >
                      {isRec && (
                      ...
                      <div className="mt-7 space-y-2">
                        ...
                        <button
                          ...
                          className="inline-flex w-full items-center justify-center gap-2 rounded-full border-2 border-primary py-2.5 text-sm font-semibold text-primary hover:bg-primary hover:text-primary-foreground transition"
                        >
        ...
        <div className="mx-auto max-w-7xl px-4 text-center">
          ...
          <Reveal variant="up" delay={1}>
            <h2 className="mb-12 text-3xl font-bold md:text-4xl">
              Unlocking Your Export Potential
            </h2>
          </Reveal>
      ...
      <PostDownloadModal
        ...
        planName={
          modalPlan
            ? `${String(modalPlan.tier).charAt(0).toUpperCase()}${String(modalPlan.tier).slice(1)} Membership form`
            : ""
        }
        message={
          modalPlan?.post_download_message ??
