...
function CertificatePage() {
  ...
  useEffect(() => {
    void (async () => {
      ...
      if (!c) { setError("Certificate not found"); return; }
      setCert(c);
      let tpl: any = null;
      if (c.template_id) {
        const { data } = await supabase.from("certificate_templates").select("*").eq("id", c.template_id).maybeSingle();
        tpl = data;
      }
      if (!tpl) {
        const { data } = await supabase.from("certificate_templates").select("*").eq("tier", c.tier).eq("is_active", true).maybeSingle();
        tpl = data;
      }
  ...
  useEffect(() => {
    ...
    setReady(false);
    renderCertificate(canvasRef.current, cert, template).then(() => setReady(true)).catch((e) => setError(e.message));
  }, [cert, template]);
  ...
  if (!cert || !template) return <div className="flex min-h-screen items-center justify-center text-muted-foreground"><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Loading…</div>;
      ...
      <div className="mx-auto max-w-5xl">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex gap-2">
            <button disabled={!ready} onClick={downloadPng} className="flex items-center gap-2 rounded-full border border-border px-5 py-2 text-sm font-semibold disabled:opacity-60">
              <Download className="h-4 w-4" /> PNG
            </button>
            <button disabled={!ready} onClick={downloadPdf} className="flex items-center gap-2 rounded-full bg-primary px-5 py-2 text-sm font-semibold text-primary-foreground disabled:opacity-60">
