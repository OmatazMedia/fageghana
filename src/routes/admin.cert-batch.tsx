...
function BatchIssue() {
  ...
  const eligible = useMemo(
    () =>
      members.filter(
        (m) =>
          !existingByUser[m.user_id] &&
          m.subscription_expiry &&
          new Date(m.subscription_expiry) > new Date(),
      ),
  ...
  function selectAll() {
    setSelected(new Set(eligible.map((m) => m.user_id)));
  }
  ...
  async function issue() {
    ...
    for (const uid of selected) {
      ...
      const expires = overrideExpiry
        ? new Date(overrideExpiry).toISOString()
        : m.subscription_expiry;
      ...
      const { error } = await supabase.from("certificates").insert({
        ...
        full_name: m.contact_name || m.company_name,
        tier: m.tier,
        expires_at: expires,
      ...
      await supabase.from("notifications").insert({
        ...
        body: "Your FAGE membership certificate has been issued. Visit your dashboard to download.",
      });
      ok++;
    }
    setBusy(false);
    ...
    >
      <div className="mb-4 flex flex-wrap items-end gap-3 rounded-xl border border-border bg-card p-4">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Defaults to each member's subscription expiry.
          </p>
        </div>
        <button
          onClick={selectAll}
        ...
        <button
          ...
          className="ml-auto flex items-center gap-1.5 rounded-full bg-primary px-5 py-2 text-sm font-semibold text-primary-foreground disabled:opacity-60"
        >
      ...
      {eligible.length === 0 ? (
        <p className="rounded-xl border border-dashed border-border p-10 text-center text-sm text-muted-foreground">
          No eligible members. Members appear here once payment is confirmed and subscription is
          active.
        </p>
      ) : (
