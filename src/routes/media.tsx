...
export const Route = createFileRoute("/media")({
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "Photos, videos, and press resources from FAGE Ghana." },
  ...
  return (
    <SiteLayout>
      <PageHero
        eyebrow="Press & Media"
        title="Media Center"
        subtitle="Your source for news, press releases, photos, videos, and media resources about FAGE and Ghana's export sector"
        imageUrl="https://images.unsplash.com/photo-1495020689067-958852a7765e?q=80&w=2069&auto=format&fit=crop"
      />
      ...
      <section className="py-16">
        <div className="mx-auto max-w-7xl px-4">
          ...
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {visible.map((m) => (
              <button
                ...
                className="group relative overflow-hidden rounded-2xl bg-muted text-left shadow-sm"
              >
                <div className="aspect-[4/3] overflow-hidden">
                  {(m.thumbnail_url || m.url) && (
                    <img src={m.thumbnail_url ?? m.url} alt={m.title} className="h-full w-full object-cover transition-transform group-hover:scale-105" />
                  )}
                </div>
                {m.media_type === "video" && (
          ...
          {visible.length === 0 && (
            <p className="py-12 text-center text-muted-foreground">No media items yet.</p>
          )}
        </div>
      </section>
      ...
      {lightbox && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 p-4" onClick={() => setLightbox(null)}>
          <button onClick={() => setLightbox(null)} className="absolute right-6 top-6 flex h-10 w-10 items-center justify-center rounded-full bg-white/10 text-white">
            <X className="h-5 w-5" />
          </button>
          <div className="max-w-5xl" onClick={(e) => e.stopPropagation()}>
            {lightbox.media_type === "photo" ? (
              <img src={lightbox.url} alt={lightbox.title} className="max-h-[80vh] rounded-xl" />
            ) : (
