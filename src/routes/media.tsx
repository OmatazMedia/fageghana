  ...
  head: () => ({
    meta: [
      { title: "Media Center — FAGE Ghana" },
      {
        name: "description",
        content:
          "News, press releases, photos, videos, and media resources about FAGE and Ghana's export sector.",
      },
              ...
              <div className="flex gap-2">
                {(["all", "photo", "video"] as const).map((f) => (
                  <button
                    key={f}
                    onClick={() => setFilter(f)}
                    className={`rounded-full px-5 py-2 text-sm font-medium capitalize transition ${filter === f ? "bg-primary text-primary-foreground" : "bg-muted hover:bg-accent"}`}
                  >
                    {f === "all" ? "All" : `${f}s`}
                  </button>
                ))}
            ...
            {visible.map((m, i) => (
              <Reveal key={m.id} variant="scale" delay={Math.min((i % 3) + 1, 4) as 1 | 2 | 3 | 4}>
                <button
                  ...
                  className="group relative overflow-hidden rounded-2xl bg-muted text-left shadow-sm w-full"
                >
