...
type News = {
  ...
  cover_image_url: string | null; category: string; author: string; published_at: string;
};
...
function NewsPage() {
  ...
  useEffect(() => {
    void supabase.from("news").select("*").eq("published", true)
      .order("published_at", { ascending: false })
      .then(({ data }) => {
        if (!data) return;
        setNews(data as News[]);
        const cats = ["All", ...Array.from(new Set((data as News[]).map(n => n.category).filter(Boolean)))];
        setCategories(cats);
      });
  }, []);
  ...
  return (
    <SiteLayout>
      <PageHero eyebrow="Updates" title="News & Blog" subtitle="Stay informed about the latest developments in Ghana's export sector" imageUrl="https://images.unsplash.com/photo-1504711434969-e33886168f5c?q=80&w=2070&auto=format&fit=crop" />
                ...
                <Reveal variant="up">
                  <Link to="/news/$slug" params={{ slug: featured.slug }}
                    ...
                    <div className="relative aspect-[16/10] overflow-hidden bg-muted md:aspect-auto">
                      ...
                      <span className="absolute left-4 top-4 rounded-full bg-primary px-3 py-1 text-xs font-bold text-white">Featured</span>
                    </div>
                    <div className="flex flex-col justify-center p-7">
                      <span className="mb-2 inline-block rounded-full bg-accent px-3 py-1 text-xs font-semibold text-primary w-fit">{featured.category}</span>
                      <h2 className="mb-3 text-xl font-bold leading-snug group-hover:text-primary transition md:text-2xl">{featured.title}</h2>
                      <p className="mb-4 line-clamp-3 text-sm text-muted-foreground">{featured.excerpt}</p>
                      <div className="flex items-center gap-4 text-xs text-muted-foreground">
                        <span className="flex items-center gap-1"><User className="h-3 w-3" />{featured.author}</span>
                        <span className="flex items-center gap-1"><Calendar className="h-3 w-3" />{new Date(featured.published_at).toLocaleDateString(undefined, { dateStyle: "medium" })}</span>
                      </div>
                      <span className="mt-4 flex items-center gap-1 text-sm font-semibold text-primary">Read more <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" /></span>
                    </div>
                  </Link>
                </Reveal>
              )}
                  ...
                  <Reveal key={n.id} variant="up" delay={((i % 2) + 1) as 1 | 2}>
                    <Link to="/news/$slug" params={{ slug: n.slug }}
                      ...
                      <div className="aspect-[16/9] overflow-hidden bg-muted">
                        {n.cover_image_url
                          ? <img src={n.cover_image_url} alt={n.title} className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" />
                          : <div className="h-full w-full bg-gradient-to-br from-primary/10 to-accent" />}
                      </div>
                      <div className="flex flex-1 flex-col p-5">
                        <div className="mb-2 flex items-center gap-2">
                          <span className="rounded-full bg-accent px-2.5 py-0.5 text-[11px] font-semibold text-primary">{n.category}</span>
                          <span className="text-[11px] text-muted-foreground">{new Date(n.published_at).toLocaleDateString(undefined, { dateStyle: "medium" })}</span>
                        </div>
                        <h3 className="mb-2 flex-1 text-base font-bold leading-snug group-hover:text-primary transition">{n.title}</h3>
            ...
            <aside className="w-full lg:w-72 xl:w-80 flex-shrink-0">
              <div className="lg:sticky lg:top-24 space-y-6">
                      ...
                      return (
                        <li key={c}>
                          <button onClick={() => setActive(c)}
                            ...
                            <span className="flex items-center gap-2"><Tag className="h-3.5 w-3.5" />{c}</span>
                            <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${active === c ? "bg-white text-primary" : "bg-muted text-muted-foreground"}`}>{count}</span>
                          </button>
                        </li>
                      );
                    })}
                ...
                <div className="rounded-2xl bg-primary p-6 text-center text-white">
                  ...
                  <p className="mb-4 text-xs text-white/80">Join Ghana's premier export federation and access exclusive resources.</p>
