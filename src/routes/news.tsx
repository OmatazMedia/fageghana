...
type News = { id: string; title: string; slug: string; excerpt: string | null; cover_image_url: string | null; category: string; author: string; published_at: string };
...
function NewsPage() {
  ...
  return (
    <SiteLayout>
      <PageHero eyebrow="Updates" title="News & Updates" subtitle="Stay informed about the latest developments in Ghana's export sector" imageUrl="https://images.unsplash.com/photo-1504711434969-e33886168f5c?q=80&w=2070&auto=format&fit=crop" />
            ...
            <Reveal variant="scale">
              <Link to="/news/$slug" params={{ slug: featured.slug }} className="group grid grid-cols-1 overflow-hidden rounded-3xl bg-card shadow-sm lg:grid-cols-2">
                <div className="relative aspect-[16/10] overflow-hidden bg-muted lg:aspect-auto">
                  {featured.cover_image_url && <img src={featured.cover_image_url} alt={featured.title} className="h-full w-full object-cover transition-transform group-hover:scale-105" />}
                ...
                <div className="flex flex-col justify-center p-8 lg:p-12">
                  ...
                  <span className="inline-flex items-center gap-2 font-semibold text-primary">Read Full Story →</span>
                </div>
              </Link>
            </Reveal>
          </div>
        ...
        <div className="mx-auto max-w-7xl px-4">
          <Reveal variant="fade">
            <div className="mb-10 flex flex-wrap gap-2">
              {categories.map((c) => (
                <button key={c} onClick={() => setActive(c)} className={`rounded-full px-5 py-2 text-sm font-medium transition ${active === c ? "bg-primary text-primary-foreground" : "bg-card text-foreground hover:bg-accent"}`}>{c}</button>
              ))}
            </div>
          </Reveal>
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            {rest.map((n, i) => (
              <Reveal key={n.id} variant="up" delay={(Math.min((i % 3) + 1, 4)) as 1|2|3|4}>
                <Link to="/news/$slug" params={{ slug: n.slug }} className="group overflow-hidden rounded-2xl bg-card shadow-sm transition hover:shadow-md block h-full">
                  ...
                  <div className="p-5">
                    <div className="mb-2 flex items-center gap-3 text-xs font-semibold uppercase tracking-wider text-primary">
                      <span>{n.category}</span><span>•</span><span>{new Date(n.published_at).toLocaleDateString()}</span>
                    </div>
            ...
            {rest.length === 0 && <p className="col-span-full py-12 text-center text-muted-foreground">No articles in this category yet.</p>}
          </div>
        </div>
      </section>
    </SiteLayout>
