...
function FormsPage() {
  ...
  const [form, setForm] = useState<{ id?: string; schema: Field[]; published: boolean } | null>(
    null,
  );
  ...
  async function load() {
    const { data } = await supabase
      ...
      .maybeSingle();
    if (data)
      setForm({ id: data.id, schema: (data.schema as Field[]) ?? [], published: data.published });
  ...
  return (
    <AdminShell
      ...
      description="Drag fields onto the canvas. Reorder by dragging the handle. Click a field to edit its properties."
      action={
        <button
          onClick={save}
              ...
              <div className="space-y-3">
                ...
                {selField.type !== "heading" && (
                  <label className="flex items-center gap-2 text-sm">
                    <input
                      type="checkbox"
                      checked={selField.required ?? false}
                ...
                <FormField label="Help text">
                  <input
                    value={selField.help ?? ""}
                    onChange={(e) => updateField(selField.id, { help: e.target.value })}
                    className={inputCls}
                  />
                </FormField>
...
function Canvas({
  ...
  return (
    <div
      ref={setNodeRef}
      className="min-h-[400px] rounded-xl border-2 border-dashed border-border bg-card p-4"
    >
      {form.schema.length === 0 ? (
        ...
        <SortableContext
          items={form.schema.map((f: Field) => f.id)}
...
function FieldPreview({ field }: { field: Field }) {
  ...
  switch (field.type) {
    case "paragraph":
      return <textarea disabled placeholder={field.placeholder} className={c} rows={2} />;
    case "select":
      return (
        <select disabled className={c}>
        ...
        <div className="mt-2 space-y-1">
          {(field.options ?? []).map((o) => (
            <label key={o} className="flex items-center gap-1 text-xs">
              <input type={field.type === "radio" ? "radio" : "checkbox"} disabled /> {o}
            </label>
          ))}
        </div>
      );
    ...
    case "heading":
      return <div className="mt-1 text-base font-bold">{field.label}</div>;
    case "file":
      return <input type="file" disabled className={c} />;
    default:
      return (
        <input
