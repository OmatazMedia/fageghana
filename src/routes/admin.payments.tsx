...
function PaymentsPage() {
  ...
  async function load() {
    const { data } = await supabase
      ...
      .select(
        "*, payment_gateways(name,provider), member_profiles!inner(member_id,contact_name,company_name,email,tier,subscription_expiry)",
      )
  ...
  async function confirm(p: any) {
    ...
    const updates: any = {
      ...
      subscription_start: profile.subscription_expiry
    ...
    await supabase
      .from("payment_submissions")
      .update({ status: "confirmed", admin_notes: notes, confirmed_at: new Date().toISOString() })
      .eq("id", p.id);
      ...
      if (tpl) {
        ...
        if (!existing) {
          ...
          await supabase.from("certificates").insert({
            ...
            template_id: tpl.id,
            member_id: memberId,
            full_name: profile.contact_name || profile.company_name,
            tier: profile.tier,
    ...
    await supabase.from("notifications").insert({
      ...
      body: `Your subscription has been extended to ${newExpiry.toLocaleDateString()}. Your certificate is now available in your dashboard.`,
    });
    toast.success("Confirmed & certificate assigned");
    await load();
  }
  ...
  async function reject(p: any) {
    ...
    await supabase.from("notifications").insert({
      ...
      body: notes || "Please review your payment and try again.",
    });
    await load();
  }
      ...
      <div className="mb-4 flex gap-2">
        {(["pending", "confirmed", "rejected", "all"] as const).map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            className={`rounded-full px-3 py-1 text-sm capitalize ${filter === f ? "bg-primary text-primary-foreground" : "bg-muted"}`}
          >
            {f}
          </button>
        ))}
            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <div className="font-semibold">
                  {p.member_profiles?.contact_name} — {p.member_profiles?.company_name}
                </div>
                <div className="text-xs text-muted-foreground">
                  {p.member_profiles?.email} · ID: {p.member_profiles?.member_id ?? "—"}
                </div>
                <div className="mt-2 text-sm">
                  {p.currency} {Number(p.amount).toLocaleString()} via{" "}
                  {p.payment_gateways?.name ?? p.method} · Ref: {p.reference || "—"}
                </div>
                {p.member_message && (
              ...
              <div className="flex flex-col items-end gap-2">
                <span
                  className={`rounded-full px-2 py-0.5 text-xs capitalize ${p.status === "confirmed" ? "bg-emerald-100 text-emerald-700" : p.status === "rejected" ? "bg-destructive/10 text-destructive" : "bg-amber-100 text-amber-700"}`}
                >
                  {p.status}
                </span>
                {p.proof_url && (
                ...
                {p.status === "pending" && (
                  <div className="flex gap-2">
                    ...
                    <button
                      onClick={() => reject(p)}
                      className="rounded-full bg-destructive px-3 py-1 text-xs text-white"
                    >
                      Reject
                    </button>
                  </div>
