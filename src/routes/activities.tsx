...
export const Route = createFileRoute("/activities")({
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "Empowering Ghana's exporters through trade missions and industry events." },
  ...
  return (
    <SiteLayout>
      <PageHero
        ...
        subtitle="Empowering Ghana's exporters through trade missions, capacity building, networking events, and industry-leading programs"
      ...
      {featured && (
        <section className="py-16">
          <div className="mx-auto max-w-7xl px-4">
            <p className="mb-3 text-center text-sm font-semibold tracking-widest text-primary">FEATURED EVENT</p>
            <h2 className="mb-10 text-center text-3xl font-bold md:text-4xl">Don't Miss This Opportunity</h2>
            <div className="grid grid-cols-1 overflow-hidden rounded-3xl bg-card shadow-sm lg:grid-cols-2">
              ...
              <div className="flex flex-col justify-center gap-4 p-8 lg:p-12">
                ...
                <div className="flex flex-wrap gap-5 text-sm text-muted-foreground">
                  {featured.event_date && (
                    <span className="flex items-center gap-2"><Calendar className="h-4 w-4 text-primary" /> {new Date(featured.event_date).toLocaleDateString(undefined, { dateStyle: "long" })}</span>
                  )}
                  {featured.location && (
                    <span className="flex items-center gap-2"><MapPin className="h-4 w-4 text-primary" /> {featured.location}</span>
                  )}
                  {featured.spots_remaining !== null && (
                    <span className="flex items-center gap-2"><Users className="h-4 w-4 text-primary" /> {featured.spots_remaining} spots remaining</span>
                  )}
                </div>
      ...
      <section className="bg-muted/40 py-16">
        <div className="mx-auto max-w-7xl px-4">
          <p className="mb-3 text-center text-sm font-semibold tracking-widest text-primary">OUR PROGRAMS</p>
          <h2 className="mb-3 text-center text-3xl font-bold md:text-4xl">Comprehensive Activity Portfolio</h2>
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            {rest.map((a) => (
              <div key={a.id} className="overflow-hidden rounded-2xl bg-card shadow-sm transition hover:shadow-md">
            ...
            {rest.length === 0 && items.length === 0 && (
              <p className="col-span-full py-12 text-center text-muted-foreground">No activities scheduled yet.</p>
            )}
          </div>
        </div>
      </section>
