    ...
    <div className="grid min-h-screen grid-cols-1 lg:grid-cols-2">
      <aside className="relative hidden flex-col justify-between overflow-hidden bg-primary p-10 text-primary-foreground lg:flex">
        ...
        <div className="relative">
          ...
          <p className="mt-10 text-xs font-semibold uppercase tracking-widest opacity-90">
            {eyebrow}
          </p>
        ...
        <ul className="relative z-10 space-y-3">
          {bullets.map((b) => (
            <li key={b} className="flex items-start gap-3 text-sm">
              <span className="mt-0.5 flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full bg-primary-foreground/20">
      ...
      <main className="flex items-center justify-center bg-background px-4 py-10">
        <div className="w-full max-w-md">
          <Link to="/" className="text-sm text-muted-foreground hover:text-primary lg:hidden">
            ← Back to site
          </Link>
          {children}
          {footer}
