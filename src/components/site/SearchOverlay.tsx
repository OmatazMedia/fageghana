...
export function SearchOverlay({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const id = setTimeout(async () => {
      ...
      (m.data ?? []).forEach((r: any) =>
        hits.push({ kind: "media", title: r.title, href: "/media" }),
      );
  ...
  const grouped: Record<Hit["kind"], Hit[]> = {
    page: [],
    news: [],
    product: [],
    activity: [],
  ...
  return (
    <div
      className="fixed inset-0 z-[200] flex items-start justify-center bg-black/70 backdrop-blur-md animate-in fade-in duration-200"
      onClick={onClose}
    >
      <div
        onClick={(e) => e.stopPropagation()}
        className="mx-4 mt-16 w-full max-w-2xl rounded-2xl border border-border bg-background shadow-2xl animate-in zoom-in-95 duration-200"
      >
        <div className="flex items-center gap-3 border-b border-border px-5 py-4">
          ...
          <input
            ...
            placeholder="Search news, products, services, events, pages…"
          ...
          {q.trim() && !loading && results.length === 0 && (
            <div className="px-3 py-10 text-center">
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Try a different keyword or browse our menu.
              </p>
            </div>
          )}
